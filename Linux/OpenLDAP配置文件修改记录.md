...
- 配置 OpenLDAP syncprov。用于复制同步数据
  ...
  changetype: modify
  add: olcSyncRepl
  olcSyncRepl: rid=103 provider=ldap://192.168.1.1 binddn="cn=admin,dc=example,dc=cn"   bindmethod=simple credentials="123456" searchbase="dc=example,dc=cn"   type=refreshAndPersist interval=00:00:00:10 retry="60 +" timeout=1 starttls=critical
  -
  add: olcLimits
  ...
  olcSyncrepl: rid=105
    ...
    bindmethod=simple
    binddn="cn=admin,dc=example,dc=cn" credentials=123456
